...
func NewPCIAddressPoolWithNetworkStatus(sriovIfaces []v1.Interface, multusNetworks []v1.Network, vmiAnnotations map[string]string) (*PCIAddressPoolWithNetworkStatus, error) {
	...
	if err := pool.mapNetworkNameToPciAddress(sriovIfaces, multusNetworks, vmiAnnotations); err != nil {
		return nil, err
	}
...
func (p *PCIAddressPoolWithNetworkStatus) mapNetworkNameToPciAddress(sriovIfaces []v1.Interface, multusNetworks []v1.Network, vmiAnnotations map[string]string) error {
	multusInterfaceNameToNetworkStatusMap, err := mapMultusInterfaceNameToNetworkStatus(vmiAnnotations)
	if err != nil {
		return err
	}
	sriovNetworkToMultusInterfaceNameMap := mapNetworkNameToMultusInterfaceName(sriovIfaces, multusNetworks)
...
func (p *PCIAddressPoolWithNetworkStatus) Pop(networkName string) (string, error) {
	...
	if !exists {
		return "", fmt.Errorf("PCI address for SR-IOV network %s does not exist", networkName)
	}
...
func mapNetworkNameToMultusInterfaceName(interfaceList []v1.Interface, networkList []v1.Network) map[string]string {
	...
	for i, network := range networkList {
		...
		if getIfaceByNetworkName(interfaceList, network.Name) != nil {
...
func mapMultusInterfaceNameToNetworkStatus(vmiAnnotations map[string]string) (map[string]networkv1.NetworkStatus, error) {
...
func getNetworkStatusList(vmiAnnotations map[string]string) ([]networkv1.NetworkStatus, error) {
	...
	err := json.Unmarshal([]byte(netStatusesJson), &networkStatusList)
